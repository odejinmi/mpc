...
$(document).ready(function() {
    ...
    $("#image-input, #image-input2, .image-input").on('change', function(e) {
        if (e.target.files[0]) {
            ...
            if (this.files[0].type.includes('image/')) {
                ...
                $imageBox.find('.upload-text2').text(formatFileSize(this.files[0].size)).show();
            }
        }
    ...
    function readImageURL(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                $(input).parents('.image-box').find('.preview-image').attr("src", e.target.result).show();
            };
    ...
    $('#upload_excel').change(function() {
        ...
        $(this).parents('.image-box').addClass('active').find('.upload-text').text(fileName);
    });

    $(".delete_image").click(function() {
        $imageBox.find('.preview-image').hide();
        $imageBox.find('.upload-icon, .upload-text, .upload-text2').show();
        $imageBox.find('.upload-text').text($imageBox.find('.upload-text').data('text'));
    ...
    $('button[type="reset"]').on('click', function() {
        $(this).parents('form').find('.preview-image, .delete_image').attr('src', '#').hide();
        $(this).parents('form').find('.upload-icon, .upload-text, .upload-text2').show();
        $(this).parents('form').find('#image-input, #image-input2, .image-input').val('');
    })
});
...
$(document).on('click', '.confirm-Toggle', function () {
    ...
    if(toggle_id === 'cash_in_hand_overflow'){
        if ($("#cash_in_hand_overflow").is(':checked')) {
            $('#cash_in_hand_overflow_restaurant_amount').removeAttr('readonly').attr('required', true);
            $('#min_amount_to_pay_restaurant').removeAttr('readonly').attr('required', true);
            $('#min_amount_to_pay_dm').removeAttr('readonly').attr('required', true);
            $('#dm_max_cash_in_hand').removeAttr('readonly').attr('required', true);
        } else {
            $('#cash_in_hand_overflow_restaurant_amount').attr('readonly', true).removeAttr('required');
            $('#min_amount_to_pay_restaurant').attr('readonly', true).removeAttr('required');
    ...
    if(toggle_id === 'customer_date_order_sratus'){
        if ($("#customer_date_order_sratus").is(':checked')) {
            $('#customer_order_date').removeAttr('readonly').attr("required", true);
        } else {
            $('#customer_order_date').attr('readonly', true).removeAttr('required');
        }
    }
