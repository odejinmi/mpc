...
class FoodController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $schedule = RestaurantSchedule::where('restaurant_id', $restaurant->id)->first();

        if ($schedule) {
            $food->available_time_starts = $schedule->opening_time;
            $food->available_time_ends = $schedule->closing_time;
        } else {
            // Handle the case where the schedule is not found
            $food->available_time_starts = null;
            $food->available_time_ends = null;
            // Optionally, log the error or throw an exception
            // Log::warning("Schedule not found for restaurant ID: {$restaurant->id}");
        }
