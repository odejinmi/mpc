    ...
    private function order_report_formatter($data): array
    {
        ...
        foreach ($data as $item) {
               $storage[] = [
                ...
                'total_item_amount' => (float)  ($item->order_amount- $item->additional_charge - $item->dm_tips-$item->total_tax_amount-$item->delivery_charge+$item->coupon_discount_amount + $item->restaurant_discount_amount),
                'item_discount' => (float)  $item->details->sum('discount_on_food'),
                'coupon_discount' => (float)  $item->coupon_discount_amount,
                'discounted_amount' => (float)  ($item->coupon_discount_amount + $item->restaurant_discount_amount),
    ...
    private function transaction_report_formatter($data): array
    {
        ...
        foreach ($data as $item) {
            ...
            $storage[] = [
                ...
                'total_item_amount' =>(float)  ($item->order['order_amount'] - $item->additional_charge  -  $item->order['dm_tips']-$item->order['delivery_charge'] - $item['tax'] + $item->order['coupon_discount_amount'] + $item->order['restaurant_discount_amount']),
                'item_discount' =>(float)  $item->order->details->sum('discount_on_food'),
                'coupon_discount' => (float) $item->order['coupon_discount_amount'],
                'discounted_amount' => (float) ($item->order['coupon_discount_amount'] + $item->order['restaurant_discount_amount']),
                'vat' => (float) $item->tax,
